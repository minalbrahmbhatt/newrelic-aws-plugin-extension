...
module NewRelicAWS
  ...
  module KINESIS
    class Agent < Base::Agent
      agent_guid "com.newrelic.aws.kinesis"
      agent_version NewRelicAWS::VERSION
  ...
  module FIREHOSE
    class Agent < Base::Agent
      agent_guid "com.newrelic.aws.firehose"
      agent_version NewRelicAWS::VERSION
      agent_human_labels("Firehose") { "Firehose" }
    end
  end
  
  module REDSHIFT
    class Agent < Base::Agent
      agent_guid "com.newrelic.aws.redshift"
      agent_version NewRelicAWS::VERSION
  ...
  module APIGATEWAY
    class Agent < Base::Agent
      agent_guid "com.newrelic.aws.apigateway"
      agent_version NewRelicAWS::VERSION
      agent_human_labels("ApiGateway") { "ApiGateway" }
    end
  end
  ...
  NewRelic::Plugin::Setup.install_agent :kinesis, KINESIS if NewRelicAWS::agent_enabled?(:kinesis)
  NewRelic::Plugin::Setup.install_agent :lambda, LAMBDA if NewRelicAWS::agent_enabled?(:lambda)
  NewRelic::Plugin::Setup.install_agent :firehose, FIREHOSE if NewRelicAWS::agent_enabled?(:firehose)
  NewRelic::Plugin::Setup.install_agent :redshift, REDSHIFT if NewRelicAWS::agent_enabled?(:redshift)
  NewRelic::Plugin::Setup.install_agent :apigateway, APIGATEWAY if NewRelicAWS::agent_enabled?(:apigateway)
